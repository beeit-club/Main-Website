...
export default function AddPhotoPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await beeitServices.photos.create(data);
      
      // Revalidate cache để cập nhật ngay lập tức
      await revalidateBeeit();
      
      toast.success("Tạo Photo thành công! Trang sẽ được cập nhật ngay lập tức.");
      router.push("/admin/beeit/photos");
    } catch (error) {
      toast.error(error?.response?.data?.message || "Tạo Photo thất bại");
      console.error(error);
    } finally {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Thêm Photo mới</h1>
          <p className="text-muted-foreground mt-2">
            Thêm hình ảnh mới vào Behind The Code
          </p>
        </div>
      </div>
