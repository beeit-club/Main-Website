...
export default function AddAchievementPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await beeitServices.achievements.create(data);
      
      // Revalidate cache để cập nhật ngay lập tức
      await revalidateBeeit();
      
      toast.success("Tạo Achievement thành công! Trang sẽ được cập nhật ngay lập tức.");
      router.push("/admin/beeit/achievements");
    } catch (error) {
      toast.error(error?.response?.data?.message || "Tạo Achievement thất bại");
      console.error(error);
    } finally {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Thêm Achievement mới</h1>
          <p className="text-muted-foreground mt-2">
            Thêm thành tựu mới vào Hall of Fame
          </p>
        </div>
      </div>
