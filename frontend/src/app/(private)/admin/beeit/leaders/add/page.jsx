...
export default function AddLeaderPage() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await beeitServices.leaders.create(data);
      
      // Revalidate cache để cập nhật ngay lập tức
      await revalidateBeeit();
      
      toast.success("Tạo Leader thành công! Trang sẽ được cập nhật ngay lập tức.");
      router.push("/admin/beeit/leaders");
    } catch (error) {
      toast.error(error?.response?.data?.message || "Tạo Leader thất bại");
      console.error(error);
    } finally {
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold">Thêm Leader mới</h1>
          <p className="text-muted-foreground mt-2">
            Thêm thông tin Cố vấn hoặc Ban Chủ Nhiệm
          </p>
        </div>
      </div>
