...
export const eventService = {
  ...
  registerEvent: async (eventId, data) => {
    try {
      const response = await axiosClient.post(`/client/events/${eventId}/registrations`, data, {
        headers: {
  ...
  checkRegistration: async (eventId) => {
    try {
      const response = await axiosClient.get(`/client/events/${eventId}/registrations/check`);
      return response.data;
    } catch (error) {
