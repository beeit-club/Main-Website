...
export function RowActions({ row, viewMode }) {
  ...
  const handleEdit = () => {
    router.push(`/admin/landing/founders/${item.id}/edit`);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      if (viewMode === "trash") {
        await founderService.deletePermanent(item.id);
        toast.success("Xóa vĩnh viễn thành công");
      } else {
        await founderService.delete(item.id);
  ...
  const handleRestore = async () => {
    try {
      await founderService.restore(item.id);
      await revalidateLanding();
  ...
  const handleToggleActive = async () => {
    setIsToggling(true);
    try {
      await founderService.toggleActive(item.id, !item.is_active);
      await revalidateLanding();
      toast.success(
