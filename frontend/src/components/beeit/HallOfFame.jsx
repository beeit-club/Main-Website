...
const HallOfFame = ({ achievements = [] }) => {
  ...
  // Log achievements khi component nhận được
  useEffect(() => {
    console.log(
      "🏆 [HALL_OF_FAME] Component received achievements:",
      achievements
    );
    console.log(
      "🏆 [HALL_OF_FAME] Achievements count:",
      achievements?.length || 0
    );
    if (achievements?.length > 0) {
      achievements.forEach((achievement, index) => {
        console.log(`🏆 [HALL_OF_FAME] Achievement ${index + 1}:`, {
          id: achievement.id,
      ...
      console.log(
        "⚠️ [HALL_OF_FAME] No achievements data, using defaultAchievements"
      );
    }
  }, [achievements]);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-8 left-8 md:top-12 md:left-12 z-20 mix-blend-difference">
        <span className="text-primary font-mono text-sm border-b border-primary pb-1 tracking-widest uppercase">
          03 // Đại_Sảnh_Vinh_Quang
        </span>
