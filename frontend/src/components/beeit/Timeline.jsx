...
const Timeline = ({ corePillars = [], events = [] }) => {
  ...
  // Ensure data exists
  if (!corePillars.length && !events.length) return null;
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div ref={coreRef} className="mb-24">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-start mb-16 border-b border-white/10 pb-12">
            ...
            <div className="md:w-2/3 space-y-6 text-gray-400 text-lg leading-relaxed">
              <p>
                <strong className="text-heading block mb-2 font-display text-xl">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {corePillars.map((pillar, idx) => (
              <div
                key={idx}
                className={`core-card p-6 rounded-xl border ${pillar.border} bg-card hover:shadow-lg transition-all duration-300 group`}
              >
                <div
        ...
        <div className="timeline-events-container relative max-w-5xl mx-auto">
          ...
          {/* Mobile Line */}
          <div className="absolute left-[19px] top-24 bottom-0 w-[2px] bg-border md:hidden z-0"></div>
          ...
          <div className="space-y-24 relative z-10">
            {events.map((event, index) => (
              ...
              >
                {/* Text Content */}
                <div
                  className={`flex-1 pl-12 md:pl-0 ${index % 2 === 0 ? "md:text-right" : "md:text-left"
                    } relative`}
                >
                  {/* Mobile Node */}
                  <div className="absolute left-0 top-1 w-10 h-10 md:hidden flex items-center justify-center">
                    <div className="w-3 h-3 bg-primary rounded-full shadow-[0_0_10px_#FFA500]"></div>
                  </div>
                ...
                <div className="w-4 h-4 rounded-full bg-background border-4 border-primary shadow-[0_0_20px_rgba(255,165,0,0.8)] z-20 shrink-0 hidden md:block"></div>
