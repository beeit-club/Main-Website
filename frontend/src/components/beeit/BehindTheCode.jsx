...
const BehindTheCode = ({ photos = [] }) => {
  ...
  // Log photos khi component nhận được
  useEffect(() => {
    console.log('📸 [BEHIND_THE_CODE] Component received photos:', photos);
    console.log('📸 [BEHIND_THE_CODE] Photos count:', photos?.length || 0);
    if (photos?.length > 0) {
      photos.forEach((photo, index) => {
        console.log(`📸 [BEHIND_THE_CODE] Photo ${index + 1}:`, {
          id: photo.id,
      ...
      console.log('⚠️ [BEHIND_THE_CODE] No photos data, using defaultPhotos');
    }
  }, [photos]);
