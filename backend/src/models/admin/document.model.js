// models/admin/document.model.js

import pool from '../../db.js';
import {
  findOne,
  insert,
  remove,
  selectWithPagination,
  update,
} from '../../utils/database.js';
import { dateTime } from '../../utils/datetime.js';

class documentModel {
  // Lấy toàn bộ
  static async getAllDocuments(options = {}) {
    let sql = `SELECT 
      d.id, 
      d.title, 
      d.slug, 
      d.description, 
      d.file_url, 
      d.preview_url, 
      d.category_id, 
      d.access_level, 
      d.status, 
      d.download_count, 
      d.created_at,
      dc.name AS category_name,
      dc.slug AS category_slug
    FROM documents d
    LEFT JOIN document_categories dc ON d.category_id = dc.id
    WHERE d.deleted_at IS NULL`;
    let params = [];

    const filters = options?.filters || {};
    if (filters.title) {
      sql += ` AND d.title LIKE ?`;
      params.push(`%${filters.title}%`);
    }
    if (filters.category_id) {
      sql += ` AND d.category_id = ?`;
      params.push(filters.category_id);
    }
    // Filter status nếu có
    if (filters.status !== undefined) {
      sql += ` AND d.status = ?`;
      params.push(filters.status);
    }

    // --- Logic phân quyền mới ---
    
    // Trường hợp 1: Lấy tài liệu dành riêng cho tôi (scope = 'private')
    if (filters.scope === 'private') {
      const roleId = filters.current_user_role_id ? parseInt(filters.current_user_role_id) : null;
      const userId = filters.current_user_id ? parseInt(filters.current_user_id) : null;

      sql += ` AND (`;
      let conditions = [];

      // Member Only
      if (roleId && roleId <= 4) {
        conditions.push(`d.access_level = 'member_only'`);
      }

      // Restricted
      if (userId) {
        conditions.push(`(d.access_level = 'restricted' AND d.id IN (SELECT document_id FROM document_restricted_users WHERE user_id = ?))`);
        params.push(userId);
      }

      // Nếu không có quyền gì (khách) nhưng lại gọi private
      if (conditions.length === 0) {
        sql += ` 1=0`; // Trả về rỗng
      } else {
        sql += conditions.join(' OR ');
      }
      
      sql += `)`;
    } 
    // Trường hợp 2: Lọc theo access_level cụ thể (ví dụ 'public' từ service getDocuments)
    else if (filters.access_level) {
      sql += ` AND d.access_level = ?`;
      params.push(filters.access_level);
    }
    // Trường hợp 3: Admin lấy tất cả (không truyền scope hay access_level cụ thể)
    else if (!filters.current_user_role_id || filters.current_user_role_id > 2) {
      // Bảo vệ: Nếu không phải Admin thì mặc định chỉ thấy Public
      sql += ` AND d.access_level = 'public'`;
    }

    return await selectWithPagination(sql, params, options);
  }

  // Kiểm tra xem user có được gán tài liệu restricted không
  static async checkUserAssignment(documentId, userId) {
    const sql = `SELECT 1 FROM document_restricted_users WHERE document_id = ? AND user_id = ? LIMIT 1`;
    const result = await findOne(sql, [documentId, userId]);
    return !!result;
  }
  // Lấy toàn bộ tài liệu đã xóa
  static async getDeletedDocuments(options = {}) {
    const sql = `SELECT id, title, slug, deleted_at FROM documents WHERE deleted_at IS NOT NULL`;
    return await selectWithPagination(sql, [], options);
  }

  // Lấy 1
  static async getOneDocument(id) {
    const sql = `SELECT
    d.*,
    dc.name AS category_name,
    dc.slug AS category_slug,
    COALESCE(assigned_data.assigned_users, JSON_ARRAY()) AS assigned_users
FROM
    documents d
LEFT JOIN document_categories dc ON d.category_id = dc.id
LEFT JOIN (
    SELECT
        dru.document_id,
        JSON_ARRAYAGG(
            JSON_OBJECT(
                'id', u.id,
                'fullname', u.fullname,
                'email', u.email,
                'avatar_url', u.avatar_url
            )
        ) AS assigned_users
    FROM
        document_restricted_users dru 
    JOIN
        users u ON dru.user_id = u.id 
    WHERE
        u.deleted_at IS NULL
    GROUP BY
        dru.document_id
) AS assigned_data ON d.id = assigned_data.document_id
WHERE
    d.id = ? AND d.deleted_at IS NULL;`;
    return await findOne(sql, [id]);
  } // Lấy 1 tài liệu đã xóa (để kiểm tra)
  // Lấy 1 tài liệu đã xóa (để kiểm tra)
  static async getOneDeletedDocument(id) {
    const sql = `SELECT id FROM documents WHERE id = ? AND deleted_at IS NOT NULL`;
    return await findOne(sql, [id]);
  }

  // Lấy 1 tài liệu theo slug
  static async getDocumentBySlug(slug) {
    const sql = `SELECT
    d.*,
    dc.name AS category_name,
    dc.slug AS category_slug
FROM
    documents d
LEFT JOIN document_categories dc ON d.category_id = dc.id
WHERE
    d.slug = ? AND d.deleted_at IS NULL`;
    return await findOne(sql, [slug]);
  }

  // Check slug đã tồn tại chưa (trừ chính nó khi update)
  static async checkIsDocument(slug, excludeId = null) {
    let sql = 'SELECT id FROM documents WHERE slug = ? AND deleted_at IS NULL';
    const params = [slug];
    if (excludeId) {
      sql += ' AND id != ?';
      params.push(excludeId);
    }
    return await findOne(sql, params);
  }

  // Thêm
  static async createDocument(docData) {
    return await insert('documents', docData);
  }

  // Cập nhật
  static async updateDocument(id, docData) {
    return await update('documents', docData, { id });
  }

  // Xóa mềm
  static async deleteDocument(id) {
    const data = { deleted_at: dateTime() };
    return await update('documents', data, { id });
  }

  // Gán người dùng (có thể gán nhiều)
  static async assignUsers(documentId, userIds) {
    // ON DUPLICATE KEY UPDATE để tránh lỗi nếu đã tồn tại cặp (document_id, user_id)
    const sql = `
      INSERT INTO document_restricted_users (document_id, user_id) 
      VALUES ? 
      ON DUPLICATE KEY UPDATE document_id=VALUES(document_id)
    `;
    const values = userIds.map((userId) => [documentId, userId]);
    const [result] = await pool.query(sql, [values]);
    return result;
  }

  // Xóa một người dùng
  static async removeUser(documentId, userId) {
    return await remove('document_restricted_users', {
      document_id: documentId,
      user_id: userId,
    });
  }
  // Khôi phục tài liệu
  static async restoreDocument(id) {
    const data = { deleted_at: null };
    return await update('documents', data, { id });
  }
}

export default documentModel;
