...
WHERE
  ...
  static async createPost(data) {
    try {
      // Nếu bài viết được tạo với trạng thái công khai (1), gán ngày xuất bản
  ...
  static async updatePost(id, data) {
    try {
      // Nếu trạng thái được cập nhật thành công khai (1), gán ngày xuất bản nếu chưa có
      if (data.status == 1 || data.status == '1') {
        const fields = Object.keys(data);
        const setClause = fields.map((f) => `\`${f}\` = ?`).join(', ');
        const sql = `UPDATE \`${table}\` SET ${setClause}, \`published_at\` = COALESCE(\`published_at\`, ?) WHERE id = ?`;
        const values = [...Object.values(data), dateTime(), id];
        const [result] = await pool.query(sql, values);
  ...
  static async changePostStatus(id, status) {
    try {
      // Nếu trạng thái được chuyển sang công khai (1), gán ngày xuất bản nếu chưa có
      if (status == 1 || status == '1') {
        const sql = `UPDATE ${table} SET status = ?, published_at = COALESCE(published_at, ?) WHERE id = ?`;
        const [result] = await pool.query(sql, [status, dateTime(), id]);
        return result;
      }
