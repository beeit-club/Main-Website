...
const documentController = {
  ...
  getMyDocuments: asyncWrapper(async (req, res) => {
    const query = PaginationSchema.cast(req.query);
    const validQuery = await PaginationSchema.validate(query, {
      stripUnknown: true,
    });

    const result = await documentClientService.getMyDocuments({
      ...validQuery,
      filters: req.query,
      user: req.user,
    });
    utils.success(res, 'Lấy danh sách tài liệu dành cho bạn thành công', result);
  }),
